 ...
 *
 * Fires when a visitor types `hire Anvesh` in the terminal.
 * Sends Anvesh an instant email via Resend so he knows someone
...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      from: FROM_EMAIL,
      to:   TO_EMAIL,
      subject: '👋 Someone typed "hire Anvesh" on your portfolio',
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; margin: 0 auto;">
          <div style="background: #0a0a0a; color: #4ade80; padding: 24px; border-radius: 12px 12px 0 0; font-family: monospace;">
            <p style="margin: 0; font-size: 13px; opacity: 0.5;">AnveshOS Terminal</p>
            <p style="margin: 8px 0 0; font-size: 16px; font-weight: 600;">$ hire Anvesh</p>
          ...
          <div style="background: #f9f9f9; padding: 24px; border-radius: 0 0 12px 12px; border: 1px solid #e5e5e5;">
            <p style="margin: 0 0 16px; font-size: 15px; color: #111; font-weight: 500;">
              Someone visited your portfolio and typed <code style="background: #f0f0f0; padding: 2px 6px; border-radius: 4px; font-size: 13px;">hire Anvesh</code> in the terminal.
            </p>
          ...
          <p style="text-align: center; color: #aaa; font-size: 11px; margin-top: 12px;">
            AnveshOS portfolio · anveshcheela.com
          </p>
        </div>
      `,
    });
