...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: TO_EMAIL,
      replyTo: email,
      subject: `[AnveshOS Contact] ${subject}`,
      html: `
        <div style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: #0a0a0a; color: #fff; padding: 24px; border-radius: 12px 12px 0 0;">
            <h1 style="margin: 0; font-size: 20px; font-weight: 600;">New message from AnveshOS</h1>
          </div>
          ...
          <p style="text-align: center; color: #aaa; font-size: 11px; margin-top: 16px;">Sent via AnveshOS portfolio</p>
        </div>
      `,
    });
